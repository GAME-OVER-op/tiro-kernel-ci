...
- Remoteproc SSR recovery: the Rust daemon enables remoteproc recovery for every PAS subsystem except `spss` (modem `remoteproc-mss`, ADSP, CDSP) as its first action on boot. The Nubia kernel hardcodes `recovery_disabled=true` for PAS subsystems (`drivers/remoteproc/qcom_q6v5_pas.c:1832`), so a subsystem firmware fatal error (observed: modem `lte_rrc_plmn_search.c:9211` assert during PLMN search) panicked the whole device into Qualcomm CrashDump. With recovery on, such events become short SSR restarts. The remoteproc core ships in the GKI Image (`CONFIG_REMOTEPROC=y`), so the runtime switch works despite the drivers being vendor modules. `scripts/kurumi_enable_modem_recovery.sh` is the standalone variant for installs that do not carry overlay.d.
